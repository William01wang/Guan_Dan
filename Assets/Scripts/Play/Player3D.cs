...
public class Player3D : MonoBehaviour
{
    ...
    public void AddCard(GameObject card_object, string cardname)
    {
        ...
        card.GetComponent<Card3D>().card_name = $"Card_{cardname}";
        ...
        else
        {
            ...
            while (play_cardnames.Count <= 0)
            {
                foreach (Transform child in transform)
                {
                    ...
                    {
                        play_cardnames.Add(child.GetComponent<Card3D>().card_name);
                    }
                }
            ...
            while (play_cardnames.Count > 0)
            {
                foreach (Transform child in transform)
                {
                    if (child.GetComponent<Card3D>().state == Card3D.CardState.OnOthers && child.GetComponent<Card3D>().card_name == play_cardnames[0])
                    {
                        if (PlayCardAction(child))
                        {
                            card_cnt--;
